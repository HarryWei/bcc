...
vcpu = affinity.pop()
_vcpu = int(vcpu)
print("I/O intensive pid: %d, running on vCPU %d" % (io_pid, _vcpu))
exiting=0
flag=0
while 1:
    try:
...
# timestamp is in microseconds.
    ...
    with open(vCPU_Sorted_RTS, 'r') as infile:
        try:
            ...
            pass
        #try:
        #    os.sched_setaffinity(pid, {vCPUs[0][1]})
        #except IndexError:
        #    pass
    #if I/O intensive thread is running on dead vCPU,
    #migrate to vCPU with maximum remaining time
    #for i in vCPUs:
    #    if int(i[1]) == _vcpu:
    #        flag = 1
    #        if int(i[0]) < int(4000):
    ...
    #            except ProcessLookupError:
    #                print("Task %d might be finished (or not I/O intensive)" % io_pid)
    #            #print("IN: I/O Thread is migrated to vCPU %d" % vCPUs[0][1])
    #if flag == 0:
    try:
        os.sched_setaffinity(io_pid, {vCPUs[0][1]})
        #_vcpu = int(vCPUs[0][1])
        ...
        pass
        #print("OUT: I/O Thread is migrated to vCPU %d" % vCPUs[0][1])
    #else:
    #    flag = 0
